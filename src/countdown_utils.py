...
def get_path_to_target(current_node, start_node):
    assert len(current_node.nums) == 1  
    target = current_node.nums[0]
    # from start node to current node
    nodes_to_current = [current_node]
    while str(current_node) != str(start_node):
        # this dumb str comparison avoids mismatch when deepcopying
        nodes_to_current.append(current_node.parent)
        current_node = current_node.parent
    search_trace = ""
    # reverse the list
    search_trace += f"Moving to Node #{start_node.idx}\n"
    nodes_to_current.reverse()
    for node, next_node in zip(nodes_to_current[:], nodes_to_current[1:]):
        search_trace += f"Current State: {target}:{node.nums}, Operations: {node.operations}\n"
        search_trace += f"Exploring Operation: {next_node.operations[-1]}, Resulting Numbers: {next_node.nums}\n"
        if len(next_node.nums) == 1 and next_node.nums[0] == target:
            search_trace += f"{next_node.nums[0]},{target} equal: Goal Reached\n"
            return search_trace
        search_trace += f"Generated Node #{next_node.idx}: {target}:{next_node.nums} Operation: {next_node.operations[-1]}\n"
        search_trace += f"Moving to Node #{next_node.idx}\n"
...
class CountdownNode:
    ...
    def __str__(self):
        return f"Node Index {self.idx}: Nums {self.nums}, Ops {self.operations}, Heuristic {self.heuristic}, Parent ID {self.parent.idx if self.parent else None}"
...
def metric_fn(search_path, mode="dt"):
    ...
    if rating == "Valid path.":
        # Find the first occurrence of "Current State" and trim everything before it
        start_idx = search_path.find("Current State")
        assert start_idx != -1, "Cannot find the initial state in a valid path."
        search_path = search_path[start_idx:]
