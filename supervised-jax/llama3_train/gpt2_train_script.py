...
def main(
    ...
    mesh = MeshShardingHelper(sharding, ['replica', 'fsdp', 'tensor'], mesh_axis_splitting=physical_axis_splitting)  # Create a 3D mesh with data, fsdp, and model parallelism axes
    with mesh.get_context():
        ...
        if 'config' in model_paths:
            config = GPT2Config.from_pretrained(model_paths['config'], **model_config_override)
        # elif 'default_config_name' in model_paths:
        #     config = GPT2Config(**GPT2_STANDARD_CONFIGS[model_paths['default_config_name']], **model_config_override)
        else:
            config = GPT2Config(**model_config_override)
        
        if config_is_temp:
            os.remove(model_paths['config'])

        model = FlaxGPT2LMHeadModel(config, dtype=activation_dtype, _do_init=False, param_dtype=param_dtype, input_shape=(bsize, 1024))
        ...
        @partial(
            ...
            args_sharding_constraint=(sharding_rules, None, PS(('replica', 'fsdp'))),
            donate_argnums=(0,),
        )
        ...
        @partial(
            ...
            out_shardings=PS(),
            args_sharding_constraint=(sharding_rules, PS(('replica', 'fsdp'))),
        )
